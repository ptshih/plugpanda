...
module.exports = Promise.method((options = {}) => {
  ...
  req.end((err, res) => {
    if (err) {
      if (err.timeout) {
        return deferred.reject(new Error(`Request timed out (${err.timeout}).`));
      }
      return deferred.reject(err);
    }

    if (!res.ok) {
      return deferred.reject(new Error(`Error with code: ${res.status}`));
    }

    console.log(`Superagent success with code: ${res.status}`);
    return deferred.resolve(res.body || res.text);
  });
